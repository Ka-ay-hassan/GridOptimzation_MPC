...
# ==========================
#pe_n = np.array([0.5, -0.2, -0.3, 0.4, -0.4])  # Adjust values as needed

# Verify power balance

## 8.1. Values from the Grid --> CAUTION: SUM(pe_n) =/ 0 --> breaks the DCPF rule
#pe_n = np.array(P_mw) # Values from the grid

# 8.2. Random Values --> SUM(pe_n) == 0
pe_n = np.random.uniform(-3, 6, num_nodes)
total_power = pe_n.sum()
pe_n -= total_power / num_nodes
