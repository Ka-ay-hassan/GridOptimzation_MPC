...
annot_kwargs = dict(textcoords='offset points', fontsize=16)
...
plt.text(0.22, (y1_X + y1_Y) / 2, r'$\Delta C_{E,1}$', va='center', color='green', fontsize=18)

plt.annotate('', (0.21, y2_X), xytext=(0.21, y2_Y), arrowprops=dict(color='blue', **arrow_kwargs))
plt.text(0.22, (y2_X + y2_Y) / 2, r'$\Delta C_{E,2}$', va='center', color='blue', fontsize=18)
...
plt.text((x1_X + x1_Y) / 2, -0.035, r'$\Delta SoC_1$', ha='center', fontsize=18)

plt.annotate('', (x2_Y, 0.02), xytext=(x2_X, 0.02), arrowprops=dict(arrowstyle='<->'))
plt.text((x2_X + x2_Y) / 2, -0.035, r'$\Delta SoC_2$', ha='center', fontsize=18)

# Labels and limits: larger axis labels and tick labels
plt.xlabel('State of Charge (SoC)', fontsize=18)
plt.ylabel('One-cycle Degradation Cost ($C_E$)', fontsize=18)
